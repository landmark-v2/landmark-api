...
public class FileService extends LoggerUtils {
    ...
    /** file 정보 가져오기 **/
    public TourReviewDTO.FileDTO load(int id) {
        return tourReviewService.findFileInfoById(id);
    }
    ...
    /** file 저장 **/
    public String store(int id, MultipartFile file) {
        try {
            if (file.isEmpty()) {
            ...
            String savedFilePath = UploadFileUtils.fileSave(rootLocation.toString(), file, id);
            ...
            if (fileDTO != null) {
                logger.info("Saved tour review [id = " + fileDTO.getId() + "] file path : " + fileDTO.getPath());
    ...
    public boolean delete(int id) {
        try {
            TourReviewDTO.FileDTO fileDTO = tourReviewService.findFileInfoById(id);
